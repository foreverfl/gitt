...
func (repo *Repo) SelfTest() (string, error) {
	if _, err := repo.db.Exec(testDropSQL); err != nil {
		return "", fmt.Errorf("predrop: %w", err)
	}
	if _, err := repo.db.Exec(testCreateSQL); err != nil {
		return "", fmt.Errorf("create: %w", err)
	}
	if _, err := repo.db.Exec(testInsertSQL, "hello"); err != nil {
		_, _ = repo.db.Exec(testDropSQL)
		return "", fmt.Errorf("insert: %w", err)
	}
	var note string
	if err := repo.db.QueryRow(testSelectSQL).Scan(&note); err != nil {
		_, _ = repo.db.Exec(testDropSQL)
